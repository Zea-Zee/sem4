...
void init_matrix(double *M) {
	int i, j;
	#pragma acc parallel loop collapse(2) present(M)
	for (i=0; i<SIZE; i++)
		for (j=0; j<SIZE; j++)
...
void mul_mv_sub(double *res, double *A, double *x, double *y) {
	int i, j;
	#pragma acc parallel loop collapse(2) present(res, A, x, y)
	for (i=0; i<SIZE; i++) {
		res[i] = -y[i];
		for (j=0; j<SIZE; j++)
...
void solve_simple_iter(double *A, double *x, double *b) {
	...
	#pragma acc enter data create(Axmb[:SIZE])
	...
	#pragma acc exit data delete(Axmb[:SIZE])
	free(Axmb);
}
...
int main() {
	...
	#pragma acc enter data create(A[:SIZE*SIZE], b[:SIZE], x[:SIZE])
	init_matrix(A);
	init_b(b);
	memset(x, 0, sizeof(double)*SIZE);
	#pragma acc update device(A[:SIZE*SIZE], b[:SIZE], x[:SIZE])

	solve_simple_iter(A, x, b);

	#pragma acc update host(x[:SIZE])
	...
	#pragma acc exit data delete(A[:SIZE*SIZE], b[:SIZE], x[:SIZE])
	free(A);
	free(b);
	free(x);
}
